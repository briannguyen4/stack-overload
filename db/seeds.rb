...
a13 = Answer.create({body: "1. how does the store know which reducer to use -> This is based entirely on the action type.<br><br>2. Do type names have to be unique? -> This is not a rule. But mostly, yes. Each action has a distinct type name and the corresponding reducer gets invoked.<br><br>3. To whom or what does the reducer pass the new state object to, the store or the action? -> The reducer does not pass the new state object anywhere. Basically, it triggers a state change event to all your react components that are listening to it. All components listening to the changed state get re-rendered, with the new version of the state, thereby updating your DOM.", question_id: q22.id, author_id: user11.id})
...
v16 = Vote.create({value: 1, user_id: user8.id, voteable: q8})
v17 = Vote.create({value: 1, user_id: user7.id, voteable: q7})
...
v22 = Vote.create({value: 1, user_id: user13.id, voteable: q16})
...
v89 = Vote.create({value: 1, user_id: user30.id, voteable: q14})
